...
//If already in recent 5 pages, move it to the front and adjust array
if (in_array($page_id, $cookie_value)){
    for($i=array_search($page_id, $cookie_value);$i<4;$i++){
        $cookie_value[$i] = $cookie_value[$i+1];
    }
}
//Else remove the oldest service (index 0) and adjust array, then add current service to index 5
else{
    $i = 0;
    for($i=0;$i<4;$i++){
        $cookie_value[$i] = $cookie_value[$i+1];
...
setcookie($cookie_name, json_encode($cookie_value), time() + (86400 * 7), "/");
?>
<html>
	<head>
		...
		<link rel="stylesheet" href="assets/css/main copy 5.css" />
	...
	<body>
		...
		<!-- One -->
			<section id="one" class="main special">
				<div class="container">
					<span class="image fit primary"><img src="images/stats.jpg" alt="" /></span>
					<div class="content">
						<header class="major">
							<h2>Battlefield 1 Weapon Stats </h2>
                            </header>
						<p>As well as each weapon by class, listed are their essential stats, including damage, accuracy, hip fire rate and more. If you want to read by rank when you get them, our Battlefield 1 unlocks page has them organised neatly, alongside advice on where to get War Bonds to use them. As well as for gameplay, weapons are also particularly useful for completing Battlefield 1's Codex entries <a href="http://symthic.com/bf1-weapon-charts?secondaries=1&misc=1&support=1&recon=1&assault=1&engineer=1&sort=Class&adsc=DESC" target="_blank">here</a></p>
					</div>
